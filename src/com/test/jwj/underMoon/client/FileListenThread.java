...
public class FileListenThread implements Runnable {
	...
	private void readMsg() {
		...
		try {
			msgType = read.read();
			if (msgType == 1) {//personal photos
				uploadPeopleFile(read.read());
				System.out.println("finish");
			}else if(msgType == 2){//meeting photos
				uploadMeetingPhotos(read.read());
	...
	public void uploadPeopleFile(int userId){
	...
	public void uploadMeetingPhotos(int meetingId){
		boolean success = false;
		String photolist = ContributesDao.getMeetingPhotosAddress(meetingId);
		int lastPhoto;
		if (photolist == null || photolist.equals("")) 
			lastPhoto = 1;
		else{
		...
		RandomAccessFile fileOutStream = null;
		try {
			String path = "D:\\images"+File.separator+"meeting" + meetingId;
			File dir = new File(path);
			if (!dir.exists()) {
				dir.mkdirs();
			}
			...
			ContributesDao.updateMeetingPhotos(meetingId,lastPhoto);
			fileOutStream.close();
			picFile = null;
			success = true;
		} catch (IOException e) {
